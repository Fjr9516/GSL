...
[nSmp,nFea] = size(data);%1123  800
if (ReducedDim > nFea) || (ReducedDim <=0)
    ReducedDim = nFea;
end


if issparse(data)%判断data矩阵储存方式是否为稀疏，是返回1，不是返回0
    data = full(data);%把数据的储存方式变为全矩阵
end
sampleMean = mean(data,1);%求矩阵平均值
data = (data - repmat(sampleMean,nSmp,1));%每个数据减去其平均值
